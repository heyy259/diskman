...
from . import DirectoryScanner, DirectoryMigrator, DirectoryCleaner, DirectoryAnalyzer
from .models import AnalysisContext


@click.group()
@click.version_option(version="0.2.0", prog_name="diskman")
def main():
    """
...
def scan(path: str, depth: int, as_json: bool):
    ...
    for info in result.directories[:20]:
        ...
        if info.link_type.value != "normal":
            link_str = f" [MIGRATED -> {info.link_target}]" if info.link_target else f" [{info.link_type.value}]"
        else:
            link_str = ""
...
def analyze(path: str, context: Optional[str]):
...
def migrate(source: str, target: str, yes: bool):
    ...
    if not yes:
        ...
        click.echo(f"  2. Delete original directory")
