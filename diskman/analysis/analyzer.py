...
class DirectoryAnalyzer:
    ...
    def _adjust_for_context(
        ...
        if context.user_type == "developer" or context.project_type:
            ...
            dev_patterns = {
                "python": [".venv", "venv", ".conda", "__pycache__", ".pytest_cache", "site-packages"],
            ...
            if context.project_type:
                ...
                if any(p in name for p in patterns):
                    if dir_type == DirectoryType.CACHE or dir_type == DirectoryType.DEPENDENCY:
                        ...
                        return dir_type, RiskLevel.LOW, RecommendedAction.CAN_DELETE, reason, confidence + 0.1
                    elif dir_type == DirectoryType.UNKNOWN:
                        return DirectoryType.DEPENDENCY, RiskLevel.LOW, RecommendedAction.CAN_DELETE, f"Development dependency for {context.project_type}", 0.7
        ...
        if context.intent == "migrate" and context.target_drive:
            ...
            if directory.size_mb > 500 and action == RecommendedAction.KEEP:
                target_path = f"{context.target_drive}\\migrated\\{path.name}"
                return dir_type, RiskLevel.MEDIUM, RecommendedAction.CAN_MOVE, f"Large directory suitable for migration to {context.target_drive}", 0.6
